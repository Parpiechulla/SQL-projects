...
-- -- Query for total deaths. 
SELECT 
    SUM(final_deaths) AS death_count
FROM (
    SELECT 
        location, 
        MAX(total_deaths) AS final_deaths
    FROM covid.deaths
    WHERE continent IS NOT NULL
    GROUP BY location
) AS t;
...
SELECT 
    SUM(worldwide_cases) AS world_cases
FROM (
    SELECT 
        location, 
        MAX(total_cases) AS worldwide_cases
    FROM covid.deaths
    WHERE continent IS NOT NULL
    GROUP BY location
) AS ta;
...
ORDER BY date
 ...
 -- Query for total tests carried out. 
SELECT 
    SUM(final_tests) AS test_count
FROM (
    SELECT 
        location, 
        MAX(total_tests) AS final_tests
...
-- Query for Total People Vaccinated 
SELECT SUM(vaccinated_people) AS vacc_people
FROM(
  SELECT location, MAX(people_vaccinated) AS vaccinated_people
  FROM covid.vaccinations
  WHERE continent IS NOT NULL
  GROUP BY location)
AS v
